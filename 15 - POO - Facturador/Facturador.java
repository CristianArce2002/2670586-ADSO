...
public class Facturador extends JFrame {
	...
	// Metodos
	public Facturador(Usuario[] lista_usuarios, Producto[] lista_productos) {
	...
	public void initComponent() {
		...
		deshabilitar_input(input_nombres_cliente);
		deshabilitar_input(input_direccion_cliente);
		deshabilitar_input(input_nombres_vendedor);
		...
		KeyListener evento_key_id = new KeyListener(){
			...
			public void keyReleased(KeyEvent e){
				...
				for (int i = 0; i < lista_productos.length; i++){
					...
					if (lista_productos[i] != null  && lista_productos[i].getId().equals(input_id_producto.getText())) {

						input_nombre_producto.setText(lista_productos[i].getNombre());
	...
	public void deshabilitar_input(JTextField campo) {
		campo.setEditable(false);
		campo.setBackground(new Color(238, 238, 238));
		campo.setForeground(Color.BLACK);
	}

	public void bucar_cliente() {

		String cedula = input_cedula_cliente.getText();
		for (int i = 0; i < this.lista_usuarios.length; i++) {
			if (lista_usuarios[i] != null && lista_usuarios[i].getRol().equals("CLIENTE") && lista_usuarios[i].getCedula().equals(cedula)) {
				input_nombres_cliente.setText(lista_usuarios[i].getNombres());
				input_direccion_cliente.setText(lista_usuarios[i].getDireccion());
				input_cedula_vendedor.requestFocus();
			}
		}
	}

	public void bucar_vendedor() {

		String cedula = input_cedula_vendedor.getText();
		for (int i = 0; i < this.lista_usuarios.length; i++) {
			if (lista_usuarios[i] != null && lista_usuarios[i].getRol().equals("Vendedor") && lista_usuarios[i].getCedula().equals(cedula)) {
				input_nombres_vendedor.setText(lista_usuarios[i].getNombres());
